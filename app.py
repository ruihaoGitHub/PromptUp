...
消极"""
            ...
            st.session_state.test_input = """标题：Sora 飓风席卷好莱坞：影视行业的毁灭性打击还是生产力革命？
正文：
2024年初，OpenAI 发布了其首款视频生成模型 Sora。该模型能够根据简单的文本描述生成长达 60 秒的高清视频，其表现出的物理世界模拟能力、复杂的摄影机运动以及极具真实感的光影效果，瞬间在全球范围内引发了巨大的震动。尤其是在影视工业的重镇好莱坞，Sora 的出现被视为继“演员工会大罢工”后的又一场行业地震。
从技术层面看，Sora 采用了名为“时空补丁”（Spacetime Patches）的技术架构。与传统的逐帧生成不同，Sora 能够理解物体在三维空间中的持久性。这意味着，当一个角色在视频中转身或被遮挡时，模型依然能记住其外观特征。这种对物理规律的深度模拟，使得 AI 生成的短片在观感上已经逼近专业摄影机拍摄的素材。
行业巨头们对此反应不一。著名制片人泰勒·派瑞（Tyler Perry）在看到 Sora 的演示后，宣布暂停其耗资 8 亿美元的制片厂扩建计划。派瑞直言不讳地表示：“在看过 Sora 的能力后，我意识到我们可能不再需要花费数千万美元去兴建实景影棚，也不再需要长途跋涉去异国取景。”
然而，对于底层的影视从业者，尤其是特效师、灯光师和初级分镜师来说，Sora 的出现更像是一场生存危机。根据一份行业调查报告显示，超过 60% 的特效从业者担心自己的岗位将在未来三年内被 AI 替代。与此同时，版权争议也愈演愈烈。好莱坞编剧工会（WGA）和演员工会（SAG-AFTRA）已经开始呼吁制定严厉的法律，防止 AI 开发者在未经许可的情况下，利用受版权保护的电影画面来训练模型。
尽管担忧重重，一些先锋创作者已经开始拥抱这一工具。独立导演们认为，Sora 极大地降低了电影制作的门槛，使得“一个人就是一个制片厂”成为可能。在过去，一个具有视觉冲击力的特效镜头可能需要数月的后期制作和数万美金的投入，而现在，通过不断优化 Prompt，几分钟内就能生成多个候选版本。
未来，影视行业的生产流程可能会发生根本性重构。导演的角色将从“现场指挥者”部分转向“AI 指令策划者”。传统的线性制作流——前期、中期拍摄、后期特效——可能会融合为一种实时的、迭代的创作模式。专家预测，第一部由 AI 生成核心素材的长篇电影可能在未来 24 个月内出现。虽然人类的创意灵魂目前仍难以被替代，但影视制作的“工具箱”已经发生了永久性的改变。"""
            st.session_state.reference_output = """事件概述：OpenAI 发布的视频生成模型 Sora 凭借 60秒超长高清视频生成能力和三维空间模拟技术，对全球影视行业产生深远冲击。
技术优势：Sora 采用“时空补丁”架构，实现了对物理规律的深度模拟和物体特性的持久记忆，视觉效果逼近专业拍摄。
行业冲击：
资本端：制片人泰勒·派瑞因 AI 潜力暂停了 8亿美元制片厂扩建计划。
就业端：约 60% 的特效从业者面临失业风险。
面临争议：版权保护成为焦点，工会组织要求限制利用版权素材训练 AI。
未来预测：影视生产流程将面临重构，制作门槛大幅降低，预计 24个月内出现首部 AI 素材长片，导演角色将向“AI 指令策划者”转型。"""
        elif task_type == "翻译任务":
...
还可以吧，没有特别的感觉。"""
        ...
        # 提示：文本框已预填充示例，用户可以直接使用或修改
        st.caption("💡 文本框已预填充示例数据，您可以直接测试或替换为自己的数据")
