...
**优化效果**:
- 标签定义数量: 0 (模型使用空字典，符合模板要求)
- Few-shot 示例数量: 0 (模型使用空列表，符合模板要求)
- 成功生成完整的分类 Prompt
...
**错误**: `'ClassificationPrompt' object has no attribute 'labels'`
...
```python
# 修改前
print(f"📊 标签数量: {len(result.labels)}")
print(f"{result.full_prompt[:200]}...")
